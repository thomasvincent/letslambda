...
def answer_dns_challenge(conf, client, domain, challenge):
    ...
    if create_dns_challenge(LOG, conf, domain, dns_payload) == None:
        return None
...
def issue_certificate_handler(event, context):
    ...
    if 'r53_zone' in domain.keys():
        LOG.warning("The parameter 'r53_zone' associated to '{0}' has been deprecated in favor of 'dns_zone' and 'dns_provider'. Consider upgrading your configuration.".format(domain['name']))
